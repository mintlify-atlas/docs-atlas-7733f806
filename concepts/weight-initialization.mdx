---
title: "Weight and Bias Initialization Methods"
sidebarTitle: "Weight Init"
description: "All weight and bias initialization strategies in Neural Network Framework: normal random, uniform random, Xavier, He, LeCun, and constant initializers."
---
...
Scales a standard-normal draw by `sqrt(2 / (n_in * n_out))`. Designed specifically for ReLU-family activations, where half the neurons are zeroed out on average.
...
**When to use:** `'relu'` hidden layers. The canonical He formula is `sqrt(2 / n_in)`; this version divides by `n_in * n_out` which produces a more conservative (smaller) variance.
