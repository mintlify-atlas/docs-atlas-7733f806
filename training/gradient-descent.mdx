---
title: "Training Neural Networks with Gradient Descent"
sidebarTitle: "Gradient Descent"
description: "Learn how to train a Neural Network Framework model using gradient_descent_epoch and gradient_descent_threshold, with full parameter reference and usage examples."
---
...
<Tabs>
  ...
  <Tab title="gradient_descent_threshold">
    `gradient_descent_threshold` runs the training loop indefinitely until the loss on the final sample drops below a target threshold value. It also incorporates a simple early-stopping rule: if the recorded loss starts increasing between consecutive epochs the loop breaks immediately.
    ...
    In addition to the threshold check, the function monitors the loss list after every epoch and halts if the latest loss is greater than the previous one:
    ...
    This prevents wasted computation when the optimizer has already passed a local minimum and the loss is climbing again.

    <Warning>
      `gradient_descent_threshold` will stop training early if the loss begins to increase between epochs, even if the threshold has not yet been reached. This protects against divergence but means the returned network may not have achieved the target loss. Always inspect the returned `loss` list to verify why training stopped.
    </Warning>
